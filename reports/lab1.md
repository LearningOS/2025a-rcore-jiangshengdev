...
请同学们可以自行测试这些内容（运行 [三个 bad 测例 (`ch2b_bad_*.rs`)](https://github.com/LearningOS/rCore-Tutorial-Test/tree/master/src/bin)），
...
写入触发了 `Trap::Exception` (`Exception::StoreFault`) 异常，查看 `QEMU` 日志可以看到：

```log
Invalid write at addr 0x0, size 1, region '(null)', reason: rejected
riscv_cpu_do_interrupt: hart:0, async:0, cause:0000000000000007, epc:0x00000000804003a4, tval:0x0000000000000000, desc=fault_store
```
...
触发了 `Trap::Exception` (`Exception::IllegalInstruction`)，查看 `QEMU` 日志可以看到：

```log
riscv_cpu_do_interrupt: hart:0, async:0, cause:0000000000000002, epc:0x00000000804003a4, tval:0x0000000000000000, desc=illegal_instruction
```
...
监督者状态 (`sstatus`) 寄存器是一个监督级 `CSR`，不在用户模式下操作

触发了 `Trap::Exception` (`Exception::IllegalInstruction`)，查看 `QEMU` 日志可以看到：

```log
riscv_cpu_do_interrupt: hart:0, async:0, cause:0000000000000002, epc:0x00000000804003a6, tval:0x0000000000000000, desc=illegal_instruction
```
...
#### 2. 深入理解 [trap.S](https://github.com/LearningOS/rCore-Tutorial-Code/blob/ch3/os/src/trap/trap.S)

中两个函数 `__alltraps` 和 `__restore` 的作用，并回答如下问题：

##### 1. L40：刚进入 `__restore` 时，`sp` 代表了什么值。请指出 `__restore` 的两种使用情景。

刚进入 `__restore` 时，上一步在执行 `switch`，`switch` 中最后将内核栈地址写入了 `sp` 寄存器
